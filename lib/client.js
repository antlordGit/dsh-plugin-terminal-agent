...
function loadAgents() {
  ...
  if (typeof localStorage !== 'undefined') {
    try {
      const saved = JSON.parse(localStorage.getItem(AGENTS_KEY) || 'null')
      if (Array.isArray(saved)) agentDefinitions = saved.map(function (agent) { return { ...agent, args: typeof agent.args === 'string' ? agent.args : '' } })
    } catch (e) {}
  }
  if (agentDefinitions === null) agentDefinitions = defaults
...
function saveAgents(next) {
  ...
  if (typeof localStorage !== 'undefined') {
    try { localStorage.setItem(AGENTS_KEY, JSON.stringify(next)) } catch (e) {}
  }
...
function TerminalBridge(props) {
  React.useEffect(function () {
    ...
    const startupKey = props.scope.sessionId + ':' + props.tabId
    const url = new URL('/sidebar/ws/terminal', location.origin)
    url.protocol = url.protocol === 'https:' ? 'wss:' : 'ws:'
    const params = new URLSearchParams({ sessionId: props.scope.sessionId, tab: props.tabId })
    if (props.scope.cwd) params.set('cwd', props.scope.cwd)
    ...
    const flushLog = function (keepalive) {
      ...
      if (logBuffer === '' || !props.scope.cwd || typeof fetch !== 'function') return
    ...
    socket.addEventListener('open', function () {
      ...
      if (!sent && props.command !== null && !startupSentKeys.has(startupKey)) {
        sent = true
        startupSentKeys.add(startupKey)
        socket.send(props.command + '\r')
      }
    ...
    return function () {
      flushLog(true)
      if (terminalSenders.get(startupKey) !== undefined) terminalSenders.delete(startupKey)
      socket.close()
    }
