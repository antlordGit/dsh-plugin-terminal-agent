...
const shortcutSrcPath = join(root, 'src', 'terminal-shortcut.js')
...
const shortcutSource = await readFile(shortcutSrcPath, 'utf8')
...
const shortcutImportRe = /^import\s*\{[\s\S]*?\}\s*from\s*'\.\/terminal-shortcut\.js'\s*\r?\n/m
...
if (!importRe.test(source) || !titleImportRe.test(source) || !userInputImportRe.test(source) || !shortcutImportRe.test(source) || !exportApplyRe.test(source) || !exportInjectRe.test(source)) {
  throw new Error('src/client.js 缺少预期的 import、apply 或 inject 导出')
}
const body = source
  .replace(importRe, '')
  .replace(titleImportRe, titleSource.replace(/^export\s+/gm, '') + '\n')
  .replace(userInputImportRe, userInputSource.replace(/^export\s+/gm, '') + '\n')
  .replace(shortcutImportRe, shortcutSource.replace(/^export\s+/gm, '') + '\n')
  .replace(exportApplyRe, 'function apply(')
  .replace(exportInjectRe, 'const inject =')
  .replace(exportDefaultRe, '')

if (/^import\s/m.test(body)) throw new Error('客户端 bundle 仍包含未内联的 import')
