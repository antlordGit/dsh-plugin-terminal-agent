# dsh-plugin-terminal-tab · 会话终端页签

在 DSH 会话顶部的“对话 / 轨迹”旁新增“终端”页签。打开后显示完整的交互式终端。
...
在设置页的「智能体」中启用或停用终端快捷入口，也可添加自定义智能体命令。

![在设置中管理终端智能体](docs/images/terminal-agent-settings.png)

### 在终端使用指定智能体

打开会话的「终端」页签，点击 `+` 后可创建普通终端，或直接选择已启用的智能体。
...
- 右上角“交接给智能体”可把当前会话交给已启用的另一智能体；完整 transcript 以只读文件保存，并附带原会话、工作目录和最近进展提示
...
```bash
dsh plugin --profile web add github:antlordGit/dsh-plugin-terminal-tab
dsh web
```
...
```bash
git clone https://github.com/antlordGit/dsh-plugin-terminal-tab.git
cd dsh-plugin-terminal-tab
npm install
npm run build
...
安装并重启后，进入任意会话；顶部出现“终端”页签即表示插件已生效。
...
npm install
npm run build
dsh plugin --profile web add 'link:/你的路径/dsh-plugin-terminal-tab'
dsh web
```
