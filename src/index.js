...
export async function appendTerminalUserInput(body) {
  if (typeof body.text !== 'string' || body.text.trim() === '') throw new Error('终端用户输入无效')
  if (body.text.length > 20_000) throw new Error('终端用户输入超过 20000 字符上限')
  if (typeof body.date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(body.date)) throw new Error('终端用户输入日期无效')
  const time = Number(body.time)
  if (!Number.isFinite(time) || time <= 0) throw new Error('终端用户输入时间无效')
        ...
        try {
          ...
          const value = req.url.endsWith('/agents')
            ...
            : req.url.endsWith('/terminal-log')
              ...
              : req.url.endsWith('/terminal-input')
                ? await appendTerminalUserInput(body)
